...
import type { CredentialForDisplayId, W3cCredentialJson, W3cIssuerJson } from './types'

import { getHostNameFromUrl, sanitizeString } from '$lib/agent/display/utils'
...
export function getCredentialForDisplay(credentialRecord: W3cCredentialRecord | SdJwtVcRecord | any) {
    if (credentialRecord instanceof SdJwtVcRecord) {
        ...
        // not really needed
        const { disclosures, jwt } = decodeSdJwtSync(credentialRecord.compactSdJwtVc, (data: string | Uint8Array, alg: string) =>
            Hasher.hash(data, alg)
        )
        const decodedPayload: Record<string, unknown> = getClaimsSync(
            jwt.payload,
            disclosures,
            (data: string | Uint8Array, alg: string) => Hasher.hash(data, alg)
        )
